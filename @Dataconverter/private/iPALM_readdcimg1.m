function [qds, cocrops]=iPALM_readdcimg1(filename,center,co_cropims)
...
ims=uint16(zeros(obj.metadata.num_rows,obj.metadata.num_columns,obj.metadata.num_frames));
...
if cutflag==0
    for ii=1:totalframes
        ims(:,:,ii)=obj.getSpecificFrames(ii)';
    end
else
    for ii=1:totalframes
        ims(:,:,ii)=obj.getSpecificFrames(ii)';
