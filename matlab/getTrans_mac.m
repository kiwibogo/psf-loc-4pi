function [sr] = getTrans_mac(sr,datapath,resultfolder)
datafiles = dir([datapath,'*.mat']);
sr.Peakthresh = 20;
sr.Iratio = [1,1,1,1];
sr.SharedParams = [0,0,1,1,1,1];
sr.Peakthresh = 20;
[res] = loc4pi_psfinv_mac(datapath,datafiles(1),resultfolder,sr);
...
% dx = movmean(res.x-res.x(:,1),1000);
% dy = movmean(res.y-res.y(:,1),1000);
% figure;
% subplot(121);plot(dx);xlabel('localizations');ylabel('dx')
% subplot(122);plot(dy);xlabel('localizations');ylabel('dy')
% legend('1','2','3','4')

sr.DataShift = [mean(dy,1);mean(dx,1)];
