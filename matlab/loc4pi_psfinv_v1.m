...
function [Res] = loc4pi_psfinv_v1(datapath,datafiles,resfolder,obj)
...
%     dipshow(cat(1,cat(2,qd1,qd2),cat(2,qd3,qd4)));
    ...
    [rois,cor,roisum] = makerois_4pi(qd1,qd2,qd3,qd4,T,thresh,bxsz,shiftxy);    
    numlocs = size(rois,3);
    ...
    iterations = 100;
    fittype = 4;
    [P1] = GPUgaussMLEv2(single(roisum),PSFsigma,iterations,fittype);
    clear GPUgaussMLEv2
    ...
    [rms,rmp,ang_ctr] = getcontrast(qd1,qd2,qd3,qd4,bxsz,res,dx,dy);

    res.t = res.cor(:,3);
    res.f = ones(size(res.sx)).*ff;
    
    shared = obj.SharedParams;
    rois(rois<0) = min(abs(rois(:)));
    if obj.updateTflag == 1
        g = obj.Transoffset; 
        ox = [];
        oy = [];
        maxf = max(res.t)+1;
        frames = res.t+(res.f-1).*maxf;
        %frames = 1;
        for ss = 1:4
            ox = cat(2,ox,g.x{ss}(frames));
            oy = cat(2,oy,g.y{ss}(frames));
        end
        dx1 = dx+ox;
        dy1 = dy+oy;
        [P,CRLB,LogL] = psfloc(obj,rois,dx1,dy1,shared);
    else
        [P,CRLB,LogL] = psfloc(obj,rois,dx,dy,shared);
    end
